...
export default function StartProjectPage(){
  return <main><Navbar/><PageHero eyebrow="Start a project" title="Tell us what you want your business to become." text="You do not need a technical specification. Share your company, your goals, what is not working today, and what a successful result would look like."/>
    <section className="bg-[#f6f9fd] px-6 py-24 md:py-32"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.72fr_1.28fr]"><div><p className="text-sm font-black uppercase tracking-[.22em] text-[#2563eb]">Start the conversation</p><h2 className="mt-5 text-4xl font-black tracking-[-.04em]">Bring the vision. We will help shape the plan.</h2><p className="mt-5 leading-7 text-[#607991]">AtlasBlake reviews the business goal first, then recommends the right pages, features, content, technology, project stages, and support level.</p><div className="mt-9 space-y-4"><ContactCard icon="mail" title="Project inquiries" value="contact@atlasblaketech.com" href="mailto:contact@atlasblaketech.com"/><ContactCard icon="map" title="Location" value="Dallas–Fort Worth, Texas"/><ContactCard icon="clock" title="First step" value="A focused conversation about the business and the desired result."/></div><div className="mt-8 rounded-2xl border border-[#bfd5ef] bg-[#edf5ff] p-6"><p className="font-black text-[#173957]">What happens next?</p><p className="mt-3 text-sm leading-6 text-[#516f8c]">Your inquiry is reviewed, the project fit and next questions are identified, and a discovery conversation can be scheduled before a written proposal is prepared.</p></div></div><ContactForm/></div></section><Footer/></main>;
}
